...
[[ -n "${TARGET_RELEASE}" ]] || release_die "release Stable anterior não localizada"
...
case "$(readlink -f "${TARGET_RELEASE}")" in
    "$(readlink -m "${RELEASE_ROOT}/stable")"/*) ;;
    *) release_die "release alvo não pertence ao canal Stable" ;;
esac
release_validate_manifest "${TARGET_RELEASE}" "${RELEASE_ID}" "${COMMIT}"
release_validate_manifest_channel "${TARGET_RELEASE}" stable
release_set_log "${LOG_ROOT}" stable
release_acquire_lock stable

release_backup_current stable "${CURRENT_LINK}" "${BACKUP_ROOT}" "${TARGET_RELEASE}/.env"
release_audit_migrations "${TARGET_RELEASE}"
release_run_tests "${TARGET_RELEASE}"
release_cli_health_check "${TARGET_RELEASE}" stable "${COMMIT}"
...
release_log "Rollback Stable concluído. O banco não foi revertido automaticamente; a release substituída foi preservada."
